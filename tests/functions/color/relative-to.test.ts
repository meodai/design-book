...
describe('relativeTo', () => {
  it('rotates hue by +180 in oklch', () => {
    ...
    ui.set('complement', relativeTo(hex('#0066cc'), 'oklch', [null, null, '+180'], ui));
  ...
  it('sets absolute lightness value', () => {
    const book = new DesignBook('test');
    const ui = book.addScope('ui');
    ui.set('dark', relativeTo(hex('#0066cc'), 'oklch', [0.3, null, null], ui));
  ...
  it('multiplies chroma by 0.5 (desaturate)', () => {
    const book = new DesignBook('test');
    const ui = book.addScope('ui');
    ui.set('muted', relativeTo(hex('#0066cc'), 'oklch', [null, '*0.5', null], ui));
  ...
  it('null preserves original channel value', () => {
    const book = new DesignBook('test');
    const ui = book.addScope('ui');
    ui.set('same', relativeTo(hex('#0066cc'), 'oklch', [null, null, null], ui));
  ...
  it('subtracts from a channel', () => {
    const book = new DesignBook('test');
    const ui = book.addScope('ui');
    ui.set('darker', relativeTo(hex('#0066cc'), 'oklch', ['-0.2', null, null], ui));
  ...
  it('divides a channel', () => {
    const book = new DesignBook('test');
    const ui = book.addScope('ui');
    ui.set('halved', relativeTo(hex('#0066cc'), 'oklch', ['/2', null, null], ui));
