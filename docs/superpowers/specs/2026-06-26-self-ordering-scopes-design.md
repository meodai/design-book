...
- **Lazy-loaded:** the library and `trained.json` are dynamically imported the first time a color ordering is computed (mirrors the `ramp()`/dittotones lazy-engine pattern), keeping them out of hot paths and initial bundles for users who never order by color. The color orderer is therefore async-capable internally; ordering computation already happens lazily on read, so this fits.
